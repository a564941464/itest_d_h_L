...
public class LabelReportController {
	...
	public static void main(String... args) throws Exception {
		ShipmentValidateResponse sresp = generateSOPLabel("TransformXMLtoPDF/ResponseXMLS/ShipmentValidateErrorResponse_2017_01_12_05_30_06_305.xml", "TransformXMLtoPDF/ResponseXMLS");

	}

	/**
	 * This method fetches different values from request xml and then populates
	 * .jasper file and finally generates .pdf file.
	 * 
	 * @param String
	...
	public static ShipmentValidateResponse generateSOPLabel(String requestXMLPath, String sopLabelPath) throws Exception {
		...
		ShipmentValidateResponse shipmentValidateResponse = (ShipmentValidateResponse) unmarshaller.unmarshal(new java.io.FileReader(requestXMLPath));// End
		...
		// Start:Populating HashMap with label values which are same for all the
		// pieces (pages)
		...
		// mary 5th Aug 2010 - remove the restriction of CNPJ which only for BR
		// - begin
		if ("BR".equalsIgnoreCase(shipmentValidateResponse.getShipper().getCountryCode())) {

			if (shipperCNPJ != null && !"".equals(shipperCNPJ.trim())) // mary
																		// 29/10/2010
																		// add
																		// the
																		// empty
		...
		if ("US".equalsIgnoreCase(shipmentValidateResponse.getShipper().getCountryCode()) && !"US".equalsIgnoreCase(shipmentValidateResponse.getConsignee().getCountryCode())) {

			shipmentDetails.put("CommerceControlStatement", "These commodities, technology or software were exported " + "from the United States in accordance with the Export Administration " + "regulations. Diversion contrary to U.S law prohibited. " + "Shipment may be varied via intermediate stopping places " + "which DHL deems appropriate.");
		}
		...
		if (shipperPostalCode != null && !"".equals(shipperPostalCode.trim())) {
			...
			// mary June 2011 move to here print the shipper postal code only if
			// it is available as there are non-postcode country
			shipperAdressDetails.append(shipperPostalCode);
		}
		// mary June 2011 move to print the shipper postal code only if it is
		// available as there are non-postcode country - begin)
		// shipperAdressDetails.append(shipperPostalCode);
		// mary June 2011 move to print the shipper postal code only if it is
		// available as there are non-postcode country - end)
		...
		if (receiverPostalCode != null && !"".equals(receiverPostalCode.trim())) {
			...
			// mary June 2011 move to here to print the shipper postal code only
			// if it is available as there are non-postcode country
		...
		// Modify on 01-Feb-2010 to move Contact Name to destination address
		// field
		...
		// Populate OutBoundSortCode, InBoundSortCode and
		...
		// If shipment is dutiable ProductContentCode should be displayed in
		// inverse video
		...
		// Modified on 01-Feb-2010 to make static text "Content:" align with
		// value
		...
		// AWB should be in seperated by space after 4 digits, staring from
		// right
		...
		// String routingBarcode =
		// "("+shipmentValidateResponse.getDHLRoutingDataId()+")" +
		// shipmentValidateResponse.getDHLRoutingCode();
		...
		// Start:Populating piece specific values in pieceMap which will be
		...
		if (serviceList != null) {
			for (SpecialService specialService : serviceList) {
				...
				if ("D".equalsIgnoreCase(specialService.getSpecialServiceType()) && (DutyTaxPaymentType.fromValue("T") == dutyPaymentType)) {
					shipmentDetails.put("DutyAccountNumber", "" + dutyAccountNumber);
				}
				// mary 5th August 2010 remove this Insurance Value which
				// populate from Insurance Charges field value - begin
				/*
				 * if
				 * ("I".equalsIgnoreCase(specialService.getSpecialServiceType(
				 * ))) { shipmentDetails.put("InsuredAmount","" +
				 * specialService.getChargeValue() + " " +
				 * specialService.getCurencyCode()); ; }
				 */
				// mary 5th August 2010 remove this Insurance Value which
				// populate from Insurance Charges field value - end

			}
		}

		// mary 5th August 2010 Insurance Value which populate from
		// InsuredAmount field value in xml request - begin
		...
		// mary 5th August 2010 Insurance Value which populate from
		// InsuredAmount field value in xml request - end
		...
		if (dutiable != null) {
			...
			// shipmentDetails.put("DeclaredValueCurrency","" +
			// shipmentValidateResponse.getDutiable().getDeclaredCurrency());
		...
		if (shipmentValidateResponse.getBilling().getBillingAccountNumber() != 0) {
			shipmentDetails.put("BillingAccountNumber", "" + shipmentValidateResponse.getBilling().getBillingAccountNumber());
		}
		// End

		// Start:Creating JRMapCollectionDataSource object and exporting the
		...
		// Updated below from GlobalLabel to TransportLabel :: Pavan Kumar
		// (TechMahindra) | 25-JUL-2013 | XMLPI 4.7 | Archive document image
		// output option
		net.sf.jasperreports.engine.JasperExportManager.exportReportToPdfFile(jasperPrint, sopLabelPath + "TransportLabel_" + shipmentValidateResponse.getAirwayBillNumber() + ".pdf");// End
